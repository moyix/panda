...
int main(int argc, char **argv) {
    ...
    pcap_activate(p);
    plugin_log = pcap_dump_open(p, argv[2]);
    if(!plugin_log) {
    ...
    while(!log_is_empty()) {
        ...
        if (log_entry->header.kind == RR_SKIPPED_CALL &&
                log_entry->variant.call_args.kind == RR_CALL_HANDLE_PACKET) {
            RR_handle_packet_args *p = &log_entry->variant.call_args.variant.handle_packet_args;
            gettimeofday(&h.ts, NULL);
            h.caplen = p->size;
            h.len = p->size;
            pcap_dump((u_char *)plugin_log, &h, p->buf);
            g_free(p->buf);
        }
